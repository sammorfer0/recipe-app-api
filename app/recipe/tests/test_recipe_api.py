...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_update_user_returns_error(self):
        """Test updating user on a recipe returns an error."""
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        ingredient_cauliflower = Ingredient.objects.create(
            user=self.user,
            name='Cauliflower',
        )
        recipe = create_recipe(user=self.user)
        ...
        ingredient_tortillas = Ingredient.objects.create(
            user=self.user,
            name='Tortillas',
        )
        payload = {'ingredients': [{'name': 'Tortillas'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(ingredient_tortillas, recipe.ingredients.all())
        self.assertNotIn(ingredient_cauliflower, recipe.ingredients.all())
